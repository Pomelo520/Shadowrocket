  ...
  const loadModules = async () => {
    ...
    await Promise.all(
      files.map(async (f) => {
        ...
        const appName = f.name.replace(/\.sgmodule$/i, "");
  ...
  const initEvents = () => {
    ...
    $("#confirmOk").onclick = () => {
      if (state.pendingInstall) {
        const f = encodeURIComponent(state.pendingInstall);
        const url = `https://xiangwanguan.github.io/Shadowrocket/Static/Redirect.html?url=shadowrocket://install?module=https://xiangwanguan.github.io/Shadowrocket/Release/Modules/${f}`;
